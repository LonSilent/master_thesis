...
void NodeType() {
  ...
  for (i = 0; i < table_size; i++) {
    ...
    //}
    if (vocab[target].word[0] == 'u') {
      v_table.push_back(target);
    } else if (vocab[target].word[0] == 'i') {
      i_table.push_back(target);
    } else if (vocab[target].word[0] == 'w') {
      f_table.push_back(target);
    } 
    //else if (vocab[target].word[0] == 'c') {
      //a_table.push_back(target);
    //}
  }
  std::cout << "table_size     " << table_size << std::endl;
  std::cout << "users count: " << v_table.size() << std::endl;
  std::cout << "items count: " << i_table.size() << std::endl;
  std::cout << "words count: " << f_table.size() << std::endl;
  //std::cout << "cates count: " << a_table.size() << std::endl;
}
